...
export default {
  ...
  methods: {
    async fetchUsers() {
      try {
        const response = await fetch('http://localhost:3333/users'); // Ensure this is the correct endpoint
    ...
    async fetchOrders() {
      try {
        const response = await fetch('http://localhost:3333/orders'); // Ensure this is the correct endpoint
        if (!response.ok) throw new Error('Network response was not ok');
        const data = await response.json();
